...
def get_epci_communes_from_csv(file_name, id, name, table_name, label, link, reference, year_data, year_cog):
    cols = ["CODGEO", "EPCI"]
...
def load_epci_communes(pool):
    table_name = "insee_epci_communes"
    cols_table = {
    ...
    keys = "PRIMARY KEY (CODGEO, year_cog) USING BTREE, KEY (EPCI) USING BTREE"
    ...
    for missing_source in zip(ms["id"], ms["name"], ms["table_name"], ms["label"], ms["link"], ms["reference"], ms["year_data"], ms["year_cog"]):
        ...
        data = get_epci_communes_from_csv(file_name, *missing_source)
