...
def download_file(id, name, table_name, label, link, reference, year_data, year_cog):
    if year_cog == "2021":
        f = {
            ...
            "url": link,
            "dir": f"data/{year_cog}",
            "zip_name": f"departement{year_cog}.zip",
            "file_name": f"departement{year_cog}.csv",
        }
        ...
        f = {
            ...
            "dir": f"data/{year_cog}",
            "zip_name": None,
            "file_name": f"v_departement_{year_cog}.csv",
        }

    return load_file(f["name"], f["url"], f["dir"], f["zip_name"], f["file_name"])


def get_cog_departements_from_csv(file_name, id, name, table_name, label, link, reference, year_data, year_cog):
...
def load_cog_departements(pool):
    table_name = "insee_cog_departements"
    cols_table = {
    ...
    keys = "PRIMARY KEY (DEP, year_cog) USING BTREE"
    ...
    for missing_source in zip(ms["id"], ms["name"], ms["table_name"], ms["label"], ms["link"], ms["reference"], ms["year_data"], ms["year_cog"]):
        ...
        data = get_cog_departements_from_csv(file_name, *missing_source)
