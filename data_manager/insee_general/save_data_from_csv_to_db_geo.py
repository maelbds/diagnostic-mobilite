...
def download_file(id, name, table_name, label, link, reference, year_data, year_cog):
    f = {
        ...
        "url": link,
        "dir": f"data/{year_cog}",
        "zip_name": f"table-appartenance-geo-communes-{year_cog[2:]}.zip",
        "file_name": f"table-appartenance-geo-communes-{year_cog[2:]}.xlsx",
    }

    return load_file(f["name"], f["url"], f["dir"], f["zip_name"], f["file_name"])


def get_geo_communes_from_csv(file_name, id, name, table_name, label, link, reference, year_data, year_cog):
    cols = ["CODGEO", "LIBGEO", "DEP", "REG", "EPCI", "ARR", "CV"]
    data = pd.read_excel(file_name, sheet_name="COM", header=5, usecols=cols, dtype="str")
...
def load_geo_communes(pool):
    table_name = "insee_geo_communes"
    cols_table = {
    ...
    keys = "PRIMARY KEY (CODGEO, year_cog) USING BTREE"
    ...
    for missing_source in zip(ms["id"], ms["name"], ms["table_name"], ms["label"], ms["link"], ms["reference"], ms["year_data"], ms["year_cog"]):
        ...
        data = get_geo_communes_from_csv(file_name, *missing_source)
