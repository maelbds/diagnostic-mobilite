...
def get_aav_from_csv(file_name, id, name, table_name, label, link, reference, year_data, year_cog):
    cols = ["AAV2020", "LIBAAV2020", "TAAV2017", "TDAAV2017", "NB_COM"]
    data = pd.read_excel(file_name, sheet_name="AAV2020", header=5, usecols=cols, dtype="str")
...
def load_aav(pool):
    ...
    keys = "PRIMARY KEY (AAV, year_data, year_cog) USING BTREE"
    ...
    for missing_source in zip(ms["id"], ms["name"], ms["table_name"], ms["label"], ms["link"], ms["reference"], ms["year_data"], ms["year_cog"]):
        ...
        data = get_aav_from_csv(file_name, *missing_source)
