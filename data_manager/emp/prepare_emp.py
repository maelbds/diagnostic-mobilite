...
def prepare_persons_with_hh(persons_with_hh):
    ...
    persons_with_hh["BTRAVTEL"] = persons_with_hh["BTRAVTEL"].fillna(value=0)
    persons_with_hh["BTRAVNBJ"] = persons_with_hh["BTRAVNBJ"].fillna(value=0)
...
def create_day_type(persons_with_hh):
    ...
    persons_with_hh["TYPEJOUR"] = persons_with_hh["MDATE_jour"].apply(lambda x: day_to_type(x))
    return persons_with_hh
...
def prepare_travels(travels):
    ...
    travels["MACCOMPM"] = travels["MACCOMPM"].fillna(value=0)
    travels["MACCOMPHM"] = travels["MACCOMPHM"].fillna(value=0)
    travels.rename(columns={"NEW_VAC_SCOL": "VAC_SCOL"}, inplace=True)
    return travels
