...
from data_manager.db_functions import load_database_not_many
...
def load_emp_persons(pool):
    table_name = "emp_persons"
    cols_table = {
    ...
    for missing_source in zip(ms["id"], ms["name"], ms["table_name"], ms["label"], ms["link"], ms["reference"], ms["year_data"], ms["year_cog"]):
        ...
        load_database_not_many(pool, table_name, persons, cols_table, keys)

        os.remove(file_name)
        save_source(pool, *missing_source)


def load_emp_travels(pool):
    table_name = "emp_travels"
    cols_table = {
        ...
        "STATUTCOM_UU_ORI": "VARCHAR(10) NULL DEFAULT NULL",
        "STATUTCOM_UU_DES": "VARCHAR(10) NULL DEFAULT NULL",
        "TAA2017_ORI": "VARCHAR(10) NULL DEFAULT NULL",
        "TAA2017_DES": "VARCHAR(10) NULL DEFAULT NULL",
        "CATCOM_AA_ORI": "VARCHAR(10) NULL DEFAULT NULL",
        "CATCOM_AA_DES": "VARCHAR(10) NULL DEFAULT NULL",
        "MEMDESAA": "VARCHAR(10) NULL DEFAULT NULL",
        "DENSITECOM_ORI": "VARCHAR(10) NULL DEFAULT NULL",
        "DENSITECOM_DES": "VARCHAR(10) NULL DEFAULT NULL",
    ...
    for missing_source in zip(ms["id"], ms["name"], ms["table_name"], ms["label"], ms["link"], ms["reference"], ms["year_data"], ms["year_cog"]):
        ...
        load_database_not_many(pool, table_name, travels, cols_table, keys)
...
if __name__ == '__main__':
    ...
    security = False
    if not security:
        load_emp_persons(None)
        load_emp_travels(None)
