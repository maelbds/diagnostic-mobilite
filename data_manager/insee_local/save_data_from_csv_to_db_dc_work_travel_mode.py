...
def get_data_from_csv():
    variables = pd.read_csv("data/2018/dossier_complet/variables_work_travel_mode.csv", sep=";", dtype=str)
...
def save_data_from_csv_to_db(data):
    ...
    def request(cur, cols):
        cur.execute("""INSERT INTO insee_work_travel_mode """ + cols_name + """ VALUES """ + values_name, cols)
...
if __name__ == "__main__":
    ...
    security = True
    if not security:
        work_travel_mode = get_data_from_csv()
        work_travel_mode = work_travel_mode.replace({np.nan: None})
        print(work_travel_mode)
        print(work_travel_mode[work_travel_mode["geo_code"]=="79048"])
        #save_data_from_csv_to_db(work_travel_mode)
