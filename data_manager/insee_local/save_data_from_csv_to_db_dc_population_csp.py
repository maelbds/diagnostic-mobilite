...
def get_population_from_csv():
    variables = pd.read_csv("data/2018/dossier_complet/variables_population_csp.csv", sep=";", dtype=str)
...
def save_population_from_csv_to_db(pop):
    ...
    values_name = "(" + ", ".join(["?" for col in pop.columns]) + ", CURRENT_TIMESTAMP, 'INSEE_RP_2018')"

    def request(cur, cols):
        cur.execute("""INSERT INTO insee_population """ + cols_name + """ VALUES """ + values_name, cols)
