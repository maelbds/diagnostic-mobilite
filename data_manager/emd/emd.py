...
from annex.data_analysis.analysis import analysis
from data_manager.emd.standardisation import standard_specific_emd, standard_indicators, standard_reason
...
def get_emd_by_id(pool, emd_id):
    ...
    travels = pd.DataFrame(result,
                           columns=["id_ind", "id_trav", "trav_nb", "reason_ori", "reason_ori_name",
                                    "reason_ori_name_fr", "id_ori",
                                    "c_geo_code_ori", "hour_ori",
                                    "reason_des", "reason_des_name", "reason_des_name_fr", "id_des", "c_geo_code_des",
                                    "hour_des",
...
def get_emd_by_id_loop_analysis(emd_id):
    result = db_request(
        """SELECT  id_ind, id_hh, w_ind, ra_id, emd_geo.geo_code, 
                   sexe, age, status, csp, nb_car, day
           FROM emd_persons
           LEFT JOIN emd_geo ON emd_persons.ra_id = emd_geo.id AND emd_persons.emd_id = emd_geo.emd_id
           WHERE emd_persons.emd_id = :emd_id""",
        {
    ...
    persons = pd.DataFrame(result, columns=["id_ind", "id_hh", "w_ind", "ra_id", "geo_code", "sexe",
                                            "age", "status", "csp", "nb_car", "day"])

    result = db_request(
        """SELECT id_ind, id_trav, trav_nb, emd_travels.reason_ori, reasons_ori.name, reasons_ori.name_fr, zone_ori, emd_geo_ori.geo_code, hour_ori, 
                   emd_travels.reason_des, reasons_des.name, reasons_des.name_fr, zone_des, emd_geo_des.geo_code, hour_des, duration, modp, modes.name_fr, distance,
                   modes_detailed.ghg_emissions_factor, modes_detailed.cost_factor
           FROM emd_travels
    ...
    persons, travels = standard_emd(None, persons, travels, "2020")

    result = db_request(
        """SELECT  CODGEO, insee_aav_communes.AAV, insee_aav.TAAV, CATEAAV
           FROM insee_aav_communes
           LEFT JOIN insee_aav ON insee_aav_communes.AAV = insee_aav.AAV
           """, {})
    aav = pd.DataFrame(result, columns=["geo_code", "aav", "tranche_aav", "cat_aav"]).set_index("geo_code")

    persons = pd.merge(persons, aav, left_on="geo_code", right_index=True, how="left").rename(columns={"tranche_aav": "t_aa", "cat_aav": "cat_aa"})
    travels = pd.merge(travels, aav, left_on="c_geo_code_ori", right_index=True, how="left").rename(columns={"tranche_aav": "tranche_aa_ori", "cat_aav": "cat_aa_ori", "aav": "aav_ori"})
    travels = pd.merge(travels, aav, left_on="c_geo_code_des", right_index=True, how="left").rename(columns={"tranche_aav": "tranche_aa_des", "cat_aav": "cat_aa_des", "aav": "aav_des"})

    print(travels)
    mask_same_aav_ori_des = travels["aav_ori"] == travels["aav_des"]
    print(mask_same_aav_ori_des.sum())
    travels.loc[mask_same_aav_ori_des, "same_aa_ori_des"] = "1"
    travels.loc[~mask_same_aav_ori_des, "same_aa_ori_des"] = "0"

    weights = persons[["id_ind", "w_trav"]].set_index("id_ind")

    travels = pd.merge(travels, weights, left_on="id_ind", right_index=True, how="left")
    return persons, travels
...
def get_emd_by_geo_codes(pool, geo_codes):
    ...
    cur.execute("""SELECT COM_AV, COM_AP, DATE_EFF
                   ...
                   WHERE TYPECOM_AP = "COM" AND YEAR(DATE_EFF) < ? AND YEAR(DATE_EFF) > ?""",
                ["2020", "2012"])  # 2020 - 2012 good pour lyon
    result = list(cur)
    ...
    cur.execute("""SELECT 
                   ...
                   FROM emd_persons
                   LEFT JOIN emd_geo ON emd_persons.ra_id = emd_geo.id AND emd_persons.emd_id = emd_geo.emd_id
                   JOIN emd_travels AS t ON emd_persons.id_ind = t.id_ind AND emd_persons.emd_id = t.emd_id
    ...
    travels["distance"] = travels["distance"].astype(float)
    print(travels)
...
if __name__ == "__main__":
    ...
    marseille_metropole = ['13001', '13002', '13003', '13005', '13007', '13008', '13009', '13012', '13013', '13014',
                           '13015', '13016', '13019', '13020', '13021', '13022', '13023', '13024', '13025', '13026',
                           '13028', '13029', '13030', '13031', '13032', '13033', '13035', '13037', '13039', '13040',
                           '13041', '13042', '13043', '13044', '13046', '13047', '13048', '13049', '13050', '13051',
                           '13053', '13054', '13055', '13056', '13059', '13060', '13062', '13063', '13069', '13070',
                           '13071', '13072', '13073', '13074', '13075', '13077', '13078', '13079', '13080', '13081',
                           '13082', '13084', '13085', '13086', '13087', '13088', '13090', '13091', '13092', '13093',
                           '13095', '13098', '13099', '13101', '13102', '13103', '13104', '13105', '13106', '13107',
                           '13109', '13110', '13111', '13112', '13113', '13114', '13115', '13117', '13118', '13119',
                           '83120', '84089']
    ...
    cc_sud_luberon = ['84147', '84133', '84113', '84084', '84076', '84026', '84024', '84014', '84010', '84002', '84151',
                      '84121', '84090', '84052', '84042', '84009']
    ca_terres_de_provence = ['13076', '13116', '13083', '13066', '13052', '13045', '13036', '13027', '13018', '13010',
                             '13089', '13067', '13064']
    ...
    marseille_baux_provence = ['13076', '13116', '13083', '13066', '13052', '13045', '13036', '13027', '13018', '13010',
                               '13089', '13067', '13064', '13100', '13094', '13057', '13068', '13038', '13065', '13058',
                               '13034', '13011', '13006']
    ...
    persons, travels, emd_id, emd_name = get_emd_by_geo_codes(pool, lyon_metropole)
