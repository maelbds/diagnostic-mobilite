...
from compute_model.years import COG
...
def compute_and_save_travels(territories):
    ...
    print(f"{n_missing_sirens} missing siren")
    ...
    for siren in missing_sirens:
        ...
        persons, travels, analysis = compute_travels_demand(geo_codes, emp_persons, emp_travels)
        print("saving")
...
def get_epcis_coverage():
    dir_path = os.path.dirname(os.path.realpath(__file__))
    os.chdir(dir_path)

    siren = pd.read_csv("all_epcis.csv", dtype=str, delimiter=";", usecols=["epci", "is_covered_by_emd", "is_emd_added"])
    return siren


def get_territories():
    siren = get_epcis_coverage()
    siren = siren.loc[siren["is_covered_by_emd"].isna()]
    siren_codes = siren["epci"].dropna().drop_duplicates().tolist()
    ...
    territories = epcis  # pd.concat([epcis, epts])
    return territories
