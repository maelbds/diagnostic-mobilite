...
from compute_model.years import COG, year_mobpro
...
def get_flows_home_work(geo_codes):
    result = db_request(
        """ SELECT  f.CODGEO_home, c1.chflieu_lat, c1.chflieu_lon, c1.centroid_lat, c1.centroid_lon, 
            ...
            AND c1.year_cog = :year_cog
            AND c2.year_cog = :year_cog
        """,
        {
            "geo_codes": geo_codes,
            "year_mobpro": year_mobpro,
            "year_cog": COG
        }
    )
...
if __name__ == '__main__':
    ...
    result = db_request(
        """ SELECT CODGEO
            ...
            WHERE EPCI IN :geocodes
        """,
        {"geocodes": ['200071579', '247100647', '200042414', '200071538']}
    )
    geocodes = pd.DataFrame(result, columns=["geo_code"])["geo_code"].to_list()
    print(geocodes)

    flows = get_flows_home_work(geocodes)
    print(flows)
    print(flows["flow"].sum())
    print((flows["flow"] * flows["distance"]).sum() / flows["flow"].sum())
