...
class MobilityWork extends React.Component {
  constructor(props){
    ...
    this.state = {
      id: "modes_work_chart",
      id2: "dist_class_work_chart",
      name_file: "parts_modales_domicile_travail",
      name_file2: "classes_distance_domicile_travail",
      status: "loading",
      data: this.init_data,
  ...
  componentDidMount(){
    let endpoints = ["mobility/focus_home_work"]
                ...
                <div className="col-6">
                  <div className="row">
                    <div className="col-12">
                      ...
                      <p className="mb-3">Ils représentent <span className="key_figure">{total_nb === "--" ? "--" : formatFigure(parseFloat(parseFloat(total_nb).toPrecision(3)))}</span> déplacements/jour. Cela équivaut à {total_dist === "--" ? "--" : formatFigure(parseFloat(parseFloat(total_dist).toPrecision(3)))} km/jour soit {formatFigure(total_dist_pers)} km/jour en moyenne par personne active qui se déplace.</p>
                      <p>Voici comment ces déplacements se décomposent par modes et classes de distance.</p>
                    </div>
                  </div>
