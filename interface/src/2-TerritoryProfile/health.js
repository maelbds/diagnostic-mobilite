...
export function getHealthElements(communes, places){
    // Health Places
    let health_places = [{type: ["médecin généraliste", "Maison de santé pluridisciplinaire"], size: p_size_1, color: c_markers[0], label: "Médecin"},
                        {type: ["pharmacie"], size: p_size_1, color: c_markers[1], label: "Pharmacie"},
                        {type: ["urgences", "Établissement santé court séjour",
                               "Établissement santé moyen séjour", "Établissement santé long séjour"], size: p_size_4, color: c_markers[2], label: "Hôpital"}]
    let places_layer = []

    for (let e_p of health_places){
      let types = e_p.type
    ...
    health_places.map((h_p)=> legend.push({type: "LegendPlace", params: {label: h_p.label, color: h_p.color, size: h_p.size+2}}))
