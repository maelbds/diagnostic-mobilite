...
function getChoroLayer(objects, selected_filters){
  ...
  let value = (e) => e["logement_carburant_pourcentage"]
  let is_available = (e) => e["logement_carburant_pourcentage"] !== null

  let choro_legend = {
    thresholds: [0, 12, 16, 20.3, 24, 28, 100],
    colors: c_gradient_reds_greens.slice(2, 8).reverse(),
    references: [
      {label: "France", value: formatFigure(20.3)},
    ],
    title: `Part des ménages en précarité énergétique mobilité ou logement`,
    unit: "%",
  ...
  let choro_layer = new ChoroLayer({
    ...
    getValue: (d) => value(d),
    getLabel: (d) => is_available(d) ? `<b>${formatFigure(value(d))}</b> %  des ménages en précarité énergétique mobilité ou logement` : "<i>donnée indisponible</i>",
    legend: choro_legend
  });
...
function getMeshElementsTable(objects, selected_filters){
  ...
  headlines.push("Part des ménages en précarité énergétique mobilité ou logement")
...
export const precariousness_fuel_home = new Indicator({
    indicator: "precariousness_fuel_home",
    label: "précarité énergétique mobilité ou logement",
    data_source: "verified",
    description: "Les ménages en situation de précarité énergétique mobilité ou logement sont les ménages sous le 3ème décile de revenu, dont les dépenses énergétiques pour le logement ou pour le carburant de la mobilité quotidienne sont supérieures à un seuil (4,5% des revenus pour les dépenses de carburant, et 8% des revenus pour les dépenses énergétiques du logement).",
