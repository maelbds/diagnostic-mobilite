...
def get_work_flows(geo_codes, mesh, year):
    ...
    if mesh == "com":
        result = db_request(
            """ SELECT  p1.CODGEO_DES, c1.chflieu_lat, c1.chflieu_lon, cog1.LIBELLE, 
                ...
                WHERE p1.CODGEO_DES IN :geo_codes
...
class WorkFlows(Resource):
    def get(self):
        ...
        log_stats("work_flows", geo_codes, mesh, year)
        message_request("work_flows", geo_codes)
        return get_work_flows(geo_codes, mesh, year)


if __name__ == '__main__':
    print(get_work_flows(["79048"], "epci", "2018"))
