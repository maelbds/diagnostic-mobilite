...
def get_bpe_grocery(geo_codes, year):
    result = db_request(
        """SELECT insee_bpe.id, p.CODGEO_DES,
            ...
            AND categories.id != 1
            AND (insee_bpe_types.id = 'B201'
            OR insee_bpe_types.id = 'B202'
            OR insee_bpe_types.id = 'B101'
            OR insee_bpe_types.id = 'B102')
            AND p.year_cog = :cog
        """,
        {
            "geo_codes": geo_codes,
...
def get_grocery(geo_codes, year):
    ...
    places = get_bpe_grocery(all_geocodes, year)
...
class Grocery(Resource):
    def get(self):
        ...
        log_stats("grocery", geo_codes, None, year)
        message_request("grocery", geo_codes)
        return get_grocery(geo_codes, year)
