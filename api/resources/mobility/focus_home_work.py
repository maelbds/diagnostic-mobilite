...
def get_focus_home_work(geo_codes):
    travels = get_travels(geo_codes)

    mask_home_work = travels["reason_name_fr"] == "domicile ↔ travail"
    travels = travels.loc[mask_home_work]
...
class FocusHomeWork(Resource):
    def get(self):
        perimeter = context_get_request.parse()
        geo_codes = perimeter.geo_codes

        log_stats("focus_home_work", geo_codes, None, None)
        message_request("focus_home_work", geo_codes)
        return get_focus_home_work(geo_codes)


if __name__ == '__main__':
    print("go")
    print(get_focus_home_work(["79270"]))
